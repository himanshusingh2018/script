...
class Expression_bedgraph:
    ...
    def induced_genes(self,expression_file,gtf):
        ...
        expression = pd.read_csv('input_data/'+self.expression_file,header=0,sep='\t',usecols=['Gene_Symbol','log2FC'])
        ...
        #print(induced_refgene_data.head(4));exit(0)
...
class Extract_induced_genes_coordinates:
    ...
    def induced_genes(self,genelist,coordinatefile):
        ...
        #print(self.genelist.split('.')[0]);exit(0)
...
class Tf_peak_tss_binding:
    ...
    def tf_peak_tss_cluster(self,filename,tf,object,filterscore):
        ...
        tf_peak_cols=['chr','start','end','peak.name','score','strand','fold_change','-log10.pValue','-log10.qValue','summit']
        tf_peak_file = pd.read_csv('input_data/'+self.filename,sep="\t",header=0,names=tf_peak_cols,usecols=['chr','start','end','score'])        #macs2 $1,$2,$3,$5
