...
class Genespromotertfbindingcluster:
    ...
    def tss_tf_binding_cluster(self,filename,gene_promoter_cluster_table):
        ...
        for index,row in gene_promoter_cluster_tf_binding_table.iterrows():
            df1 = gene_peak_data.loc[gene_peak_data['gene'].isin(row['genes in cluster'].split(', '))].groupby('gene').distance.agg(min).reset_index()
            tf_binding_gene_promoter = ', '.join(df1['gene'].tolist())
            if(tf_binding_gene_promoter):
                gene_promoter_cluster_tf_binding_table.loc[index,'TF binding to gene promoter'] = tf_binding_gene_promoter
            else:
                gene_promoter_cluster_tf_binding_table.loc[index,'TF binding to gene promoter'] = ''
